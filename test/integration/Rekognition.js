...
const debug = require('debug')('rekognition-node:test:Rekognition');
...
describe('Rekognition', function() {
  ...
  it('uploadToS3 should response ok', async function() {
    ...
    const s3Images = await rekognition.uploadToS3(
      imagePaths,
      parameters.defaultFolder
    );
  ...
  it('index faces of Mark_Zuckerberg_and_wife2 should response ok', async function() {
    const facesIndexed = await rekognition.indexFaces(
      collectionId,
      targetImageFile
    );
  ...
  it('search faces by faceId should response ok', async function() {
    ...
    const faceMatches = await rekognition.searchFacesByFaceId(
      collectionId,
      newFaceId
    );
  ...
  it('search faces by image should response ok', async function() {
    const faceMatches = await rekognition.searchFacesByImage(
      collectionId,
      sourceImageFile
    );
