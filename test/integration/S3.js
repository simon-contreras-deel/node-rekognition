...
const debug = require('debug')('rekognition-node:test:S3');
...
describe('S3', function() {
  ...
  it('upload multiple files should be ok and ordered', async function() {
    ...
    const imagePaths = fileNames.map(
      fileName => __dirname + '/../../images/' + fileName
    );

    const s3Images = await s3.uploadMultiple(
      imagePaths,
      parameters.defaultFolder
    );

    // s3Image.Key = parameters.defaultFolder + new Date().getTime() + '-' + fileName
    const indexOfFileName =
      parameters.defaultFolder.length + (new Date().getTime() + '-').length;
    ...
    s3Images.forEach((s3Image, index) => {
      ...
      expect(s3Image).to.have.property('Bucket');
      expect(s3Image.Bucket).to.be.equal(parameters.AWS.bucket);
      expect(s3Image.Key.indexOf(fileNames[index])).to.be.equal(
        indexOfFileName
      );
    });
  });
